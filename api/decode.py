...
def decode_course_list(_text):
    console.log("[bold green]开始解码课程列表...[/bold green]")
    _soup = BeautifulSoup(_text, "lxml")
    _raw_courses = _soup.select("div.course")
    _course_list = list()

    # table = Table(title="Course Details", show_lines=True)
    table = Table(show_header=True, header_style="bold green", title_style='yellow', show_lines=True)

    table.add_column("Course ID", style="cyan", justify="center")
    table.add_column("Title", style="magenta", justify="center")
    table.add_column("Teacher", style="green", justify="center")
    ...
    for course in _raw_courses:
        if not course.select_one("a.not-open-tip") and not course.select_one(
            ...
            table.add_row(
                # _course_detail["id"],
                # _course_detail["info"],
                # _course_detail["roleid"],
                # _course_detail["clazzId"],
                _course_detail["courseId"],
                # _course_detail["cpi"],
                _course_detail["title"],
                # _course_detail["desc"],
                _course_detail["teacher"],
            )
